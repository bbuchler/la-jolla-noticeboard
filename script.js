...
(function () {
    ...
    async function loadData() {
        try {
            const response = await fetch('site-data.json?v=20260827-3');
    ...
    function renderContact() {
        ...
        container.innerHTML = siteData.contacts.map(function (c) {
            ...
            var leadershipRole = c.leadershipRole ? '<p class="contact-role contact-leadership-role">' + t(c.leadershipRole) + '</p>' : '';
            var profileLink = c.profileUrl
                ? '<a href="' + c.profileUrl + '" class="contact-profile-link">' + t(c.profileLabel) + '</a>'
                : '';
            return '<div class="contact-card">' +
                '<h3>' + c.name + '</h3>' + role + leadershipRole +
